...
이 경우는 대부분 ICP비안을 발급받은 도메인이 없다는 가정을 전제로 합니다. 만약 ICP비안 도메인을 가지고 있다면(또는 발급 받는다면) GA에서 중국-한국 다이렉트 연결을 통한 가속화가 가능합니다. 
12. GA(중국 가속화 IP사용) + pac
소개:
사용자 PC나 모바일 단말기의 OS나 브라우저에 pac을 설정하여 웹사이트 도메인에 대한 라우팅 경로를 임의로 바꿔주는 설정입니다. 웹사이트의 도메인이 ICP 비안을 발급받지 않았기 때문에 중국내 가속화 IP와 도메인을 DNS서버에 연결할 수 없습니다. 대신, client단에서 pac file이 위치한 OSS url만 설정해 주면 웹서버로 가는 경로가 중국과 한국을 다이렉트로 연결하는 알리바바 글로벌 네트워크로 변경되면서 가속화됩니다. 
다이어그램:
...
-->
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5ODM1MzA4MDYsLTMxMzk4OTI0MCwtMT
AwOTk3MjkxNSwxNzQxNTA0MDYsLTMxMzc5MzE5OSwtMTU1MjEy
MTYzLDQyNDczNjI4NywtMTMxMTg3NTIxNF19
-->